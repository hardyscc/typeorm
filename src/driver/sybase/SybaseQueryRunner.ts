...
import { SybaseDriver } from "./SybaseDriver";
import { SybaseParameter } from "./SybaseParameter";
...
export class SybaseQueryRunner extends BaseQueryRunner implements QueryRunner {
    ...
    constructor(driver: SybaseDriver, mode: "master" | "slave" = "master") {
        super();
        this.driver = driver;
        ...
        const promise = new Promise(async (ok, fail) => {
            try {
                ...
                request.query(query, (err: any, result: any) => {
                    ...
                    const resolveChain = () => {
                        if (promiseIndex !== -1)
                            this.queryResponsibilityChain.splice(
                                promiseIndex,
                                1
                            );
                    ...
                    let promiseIndex = this.queryResponsibilityChain.indexOf(
                        promise
                    );
                    let waitingPromiseIndex = this.queryResponsibilityChain.indexOf(
                        waitingPromise
                    );
                    if (err) {
    ...
    async hasTable(tableOrName: Table | string): Promise<boolean> {
        ...
        const schema =
            parsedTableName.schema === "SCHEMA_NAME()"
    ...
    async hasColumn(
        ...
        const sql = `SELECT * FROM "${
            ...
            parsedTableName.tableName
        }' AND "COLUMN_NAME" = '${columnName}' AND "TABLE_SCHEMA" = ${schema}`;
        const result = await this.query(sql);
        return result.length ? true : false;
    }
    ...
    async renameTable(
        ...
        // rename table
        upQueries.push(
            `EXEC sp_rename "${this.escapeTableName(
                oldTable,
                true
            )}", "${newTableName}"`
        );
        ...
        newTable.uniques.forEach(unique => {
            ...
            // build queries
            upQueries.push(
                `EXEC sp_rename "${this.escapeTableName(newTable, true)}.${
                    unique.name
                }", "${newUniqueName}"`
            );
            downQueries.push(
    ...
    async addColumn(
        ...
        upQueries.push(
            `ALTER TABLE ${this.escapeTableName(
                table
            )} ADD ${this.buildCreateColumnSql(table, column, false, false)}`
        );
        downQueries.push(
        ...
        if (column.isUnique) {
            ...
            downQueries.push(
                `ALTER TABLE ${this.escapeTableName(table)} DROP CONSTRAINT "${
                    uniqueConstraint.name
                }"`
            );
        }
        ...
        if (column.default !== null && column.default !== undefined) {
            const defaultName = this.connection.namingStrategy.defaultConstraintName(
                table.name,
                column.name
            );
            upQueries.push(
    ...
    async changeColumn(
        ...
        if (
            ...
            if (newColumn.name !== oldColumn.name) {
                ...
                // rename the column
                upQueries.push(
                    `EXEC sp_rename "${this.escapeTableName(table, true)}.${
                        oldColumn.name
                    }", "${newColumn.name}"`
                );
                downQueries.push(
                ...
                clonedTable.findColumnChecks(oldColumn).forEach(check => {
                    ...
                    const newCheckName = this.connection.namingStrategy.checkConstraintName(
                        clonedTable,
                        check.expression!
                    );
    ...
    async dropColumn(
        ...
        if (!column)
            throw new Error(
                `Column "${columnOrName}" was not found in table "${
                    table.name
                }"`
            );
    ...
    async dropUniqueConstraint(
        ...
        if (!uniqueConstraint)
            throw new Error(
                `Supplied unique constraint was not found in table ${
                    table.name
                }`
            );
            ...
            await Promise.all(
                allTablesResults.map(async tablesResult => {
                    ...
                    const dropForeignKeySql =
                        `SELECT 'ALTER TABLE "${
                            tablesResult["TABLE_CATALOG"]
                        }"."' + OBJECT_SCHEMA_NAME("fk"."parent_object_id", DB_ID('${
                            tablesResult["TABLE_CATALOG"]
                        }')) + '"."' + OBJECT_NAME("fk"."parent_object_id", DB_ID('${
                            tablesResult["TABLE_CATALOG"]
                        }')) + '" ` +
                        `DROP CONSTRAINT "' + "fk"."name" + '"' as "query" FROM "${
                            tablesResult["TABLE_CATALOG"]
                        }"."sys"."foreign_keys" AS "fk" ` +
                        `WHERE "fk"."referenced_object_id" = OBJECT_ID('"${
                            tablesResult["TABLE_CATALOG"]
                        }"."${tablesResult["TABLE_SCHEMA"]}"."${
                            tablesResult["TABLE_NAME"]
                        }"')`;
    ...
    protected async loadTables(tableNames: string[]): Promise<Table[]> {
        ...
        const constraintsCondition = tableNames
            .map(tableName => {
                const [schema, name] = extractTableSchemaAndName(tableName);
                return (
                    `("columnUsages"."TABLE_SCHEMA" = '${schema}' AND "columnUsages"."TABLE_NAME" = '${name}' ` +
                    `AND "tableConstraints"."TABLE_SCHEMA" = '${schema}' AND "tableConstraints"."TABLE_NAME" = '${name}')`
                );
            })
            .join(" OR ");

        const constraintsSql = dbNames
            .map(dbName => {
                return (
        ...
        const foreignKeysSql = dbNames
            .map(dbName => {
                return (
                    `SELECT "fk"."name" AS "FK_NAME", '${dbName}' AS "TABLE_CATALOG", "s1"."name" AS "TABLE_SCHEMA", "t1"."name" AS "TABLE_NAME", ` +
        ...
        const identityColumnsSql = dbNames
            .map(dbName => {
                return (
        ...
        const indicesSql = dbNames
            .map(dbName => {
                return (
                    `SELECT '${dbName}' AS "TABLE_CATALOG", "s"."name" AS "TABLE_SCHEMA", "t"."name" AS "TABLE_NAME", ` +
        ...
        if (table.checks.length > 0) {
            const checksSql = table.checks
                .map(check => {
                    ...
                    return `CONSTRAINT "${checkName}" CHECK (${
                        check.expression
                    })`;
                })
                .join(", ");
    ...
    protected dropTableSql(
        ...
        return ifExist
            ? `DROP TABLE IF EXISTS ${this.escapeTableName(tableOrName)}`
            : `DROP TABLE ${this.escapeTableName(tableOrName)}`;
    }
    ...
    protected createUniqueConstraintSql(
        ...
        const columnNames = uniqueConstraint.columnNames
            .map(column => `"` + column + `"`)
            .join(", ");
        return `ALTER TABLE ${this.escapeTableName(table)} ADD CONSTRAINT "${
            uniqueConstraint.name
        }" UNIQUE (${columnNames})`;
    }
     ...
     */
    protected mssqlParameterToNativeParameter(parameter: SybaseParameter): any {
        switch (this.driver.normalizeType({ type: parameter.type as any })) {
            case "bit":
                return this.driver.sybase.Bit;
            case "bigint":
                return this.driver.sybase.BigInt;
            case "decimal":
                return this.driver.sybase.Decimal(...parameter.params);
            case "float":
                return this.driver.sybase.Float;
            case "int":
                return this.driver.sybase.Int;
            case "money":
                return this.driver.sybase.Money;
            case "numeric":
                return this.driver.sybase.Numeric(...parameter.params);
            case "smallint":
                return this.driver.sybase.SmallInt;
            case "smallmoney":
                return this.driver.sybase.SmallMoney;
            case "real":
                return this.driver.sybase.Real;
            case "tinyint":
                return this.driver.sybase.TinyInt;
            case "char":
                return this.driver.sybase.Char(...parameter.params);
            case "nchar":
                return this.driver.sybase.NChar(...parameter.params);
            case "text":
                return this.driver.sybase.Text;
            case "ntext":
                return this.driver.sybase.Ntext;
            case "varchar":
            ...
            case "time":
                return this.driver.sybase.Time(...parameter.params);
            case "date":
                return this.driver.sybase.Date;
            case "datetime":
                return this.driver.sybase.DateTime;
            case "datetime2":
                return this.driver.sybase.DateTime2(...parameter.params);
            case "datetimeoffset":
                return this.driver.sybase.DateTimeOffset(...parameter.params);
            case "smalldatetime":
                return this.driver.sybase.SmallDateTime;
            case "uniqueidentifier":
                return this.driver.sybase.UniqueIdentifier;
            case "variant":
                return this.driver.sybase.Variant;
            case "binary":
                return this.driver.sybase.Binary;
            case "varbinary":
                return this.driver.sybase.VarBinary(...parameter.params);
            case "image":
                return this.driver.sybase.Image;
            case "udt":
                return this.driver.sybase.UDT;
            case "rowversion":
                return this.driver.sybase.RowVersion;
        }
    }
    ...
    convertIsolationLevel(isolation: IsolationLevel) {
        const ISOLATION_LEVEL = this.driver.sybase.ISOLATION_LEVEL;
        switch (isolation) {
