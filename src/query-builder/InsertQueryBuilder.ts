...
export class InsertQueryBuilder<Entity> extends QueryBuilder<Entity> {
    ...
    orUpdate(statement?: {
        ...
        if (statement && typeof statement.conflict_target === "string")
            this.expressionMap.onUpdate.conflict = ` ON CONSTRAINT ${
                statement.conflict_target
            } `;
    ...
    protected createValuesExpression(): string {
        ...
        if (columns.length > 0) {
            ...
            return valueSets
                .map((valueSet, insertionIndex) => {
                    const columnValues = Object.keys(valueSet)
                        .map(columnName => {
                            const paramName =
                                "i" + insertionIndex + "_" + columnName;
    ...
    protected getValueSets(): ObjectLiteral[] {
        if (
            this.expressionMap.valuesSet instanceof Array &&
            this.expressionMap.valuesSet.length > 0
        )
